...
function addVenueMarkers(mapboxgl) {
  ...
  for (const venue of plotted) {
    ...
    const name = escapeHtml(venue.venuename)
    const popupHtml = `<div class="place-map-popup__inner"><a href="${href}" class="place-map-popup__link"><strong>${name}</strong></a></div>`

    const marker = new mapboxgl.Marker({ color: '#d97706' })
      .setLngLat([venue.lng, venue.lat])
      .setPopup(
        new mapboxgl.Popup({ offset: 24, className: 'place-map-popup' }).setHTML(popupHtml),
      )
      .addTo(mapInstance)
  ...
  if (plotted.length === 1) {
    ...
    mapInstance.fitBounds(bounds, { padding: 48, maxZoom: plotted.length > 20 ? 10 : 14 })
  }
}
