<template>
  <section class="space-y-3">
    <div class="flex flex-wrap items-center justify-between gap-2">
      ...
      <label
        v-if="canAddMore"
        class="inline-flex cursor-pointer items-center gap-2 rounded-lg border border-primary-300 bg-primary-50 px-3 py-2 text-sm font-medium text-primary-900 hover:bg-primary-100 dark:border-primary-800 dark:bg-primary-950 dark:text-primary-100"
    ...
    <div v-if="uploading" class="space-y-1">
      ...
      <div class="h-2 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-gray-700">
        <div
          class="h-full rounded-full bg-primary-500 transition-all duration-150 ease-out"
    ...
    <ul v-if="items.length" class="grid grid-cols-1 gap-3 sm:grid-cols-2 list-none p-0 m-0">
      <li
        ...
        :class="{
          'ring-2 ring-primary-400 opacity-70': dragIndex === index,
          'border-primary-400': dropIndex === index && dragIndex !== index,
        }"
        draggable="true"
