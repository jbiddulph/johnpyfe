  ...
  <div class="mx-auto max-w-5xl px-4 py-8">
    <div class="mb-6 flex flex-wrap items-start justify-between gap-3">
      ...
      <div class="flex flex-wrap gap-2">
        <UButton color="primary" to="/map" label="Open map builder" />
    ...
    <template v-else>
      ...
      <section v-if="pendingInvites.length" class="mb-8 space-y-3">
        <div class="rounded-lg border border-primary-300 bg-primary-50 px-4 py-3 dark:border-primary-800 dark:bg-primary-950/40">
          <h2 class="text-lg font-semibold text-primary-950 dark:text-primary-100">
            {{ pendingInvites.length === 1 ? 'You have been invited' : `You have ${pendingInvites.length} invitations` }}
          </h2>
          <p class="mt-1 text-sm text-primary-900/80 dark:text-primary-200/80">
        ...
        <UCard
          ...
          :key="invite.id"
          :ui="{ body: { padding: 'p-4' }, ring: 'ring-1 ring-primary-300 dark:ring-primary-800' }"
        >
          <div class="flex flex-wrap items-center justify-between gap-3">
            <div class="min-w-0 space-y-1">
              ...
              <p v-if="invite.startsAt" class="text-sm font-medium text-primary-950 dark:text-primary-100">
                Starts {{ formatCrawlStartsAt(invite.startsAt) }}
              </p>
              <p
                v-if="invite.inviteeNotes"
                class="whitespace-pre-wrap text-sm text-primary-900/90 dark:text-primary-100/90"
              >
                {{ invite.inviteeNotes }}
              </p>
            </div>
      ...
      <section v-if="unreadNotifications.length" class="mb-8 space-y-2">
        <div class="flex items-center justify-between gap-2">
          <h2 class="text-sm font-semibold uppercase tracking-wide text-gray-500">
            Notifications
            <span class="ml-1 rounded bg-primary-100 px-1.5 py-0.5 text-primary-900">
        ...
        <ul class="space-y-2">
          <li
            ...
            :key="note.id"
            class="rounded-md border border-primary-300 bg-primary-50 px-3 py-2 text-sm dark:border-primary-800 dark:bg-primary-950/40"
          >
      ...
      <section class="mb-8 space-y-3">
        ...
        <p v-else class="text-sm text-gray-500">
          ...
          <NuxtLink to="/map" class="text-primary-700 hover:underline">Create one on the map</NuxtLink>
        </p>
      </section>
