...
-- Safe to re-run: replaces the function/trigger, backfills missing rows,
-- and repairs usernames that were generated before the lower() fix.
-- =============================================================================

CREATE OR REPLACE FUNCTION public.ukpubs_slugify_username(raw text)
RETURNS text
LANGUAGE plpgsql
IMMUTABLE
AS $$
DECLARE
  base_username text;
BEGIN
  -- Lowercase FIRST so capitals are kept as letters, not stripped as separators
  base_username := lower(coalesce(raw, ''));
...
BEGIN
  ...
  IF EXISTS (
    ...
    -- Keep display name in sync with auth metadata
    UPDATE public.ukpubs_profiles
    SET display_name = display,
        updated_at = NOW()
    WHERE user_id = p_user_id::text
      AND display_name IS DISTINCT FROM display;
    RETURN;
  END IF;

  base_username := public.ukpubs_slugify_username(display);
...
-- Repair usernames generated by the old bug (capitals were stripped as separators)
-- e.g. "Johannes Michaelle Biddaers" -> "ohannes_ichaelle_iddaers"
-- should become "johannes_michaelle_biddaers" (truncated to 24 chars).
DO $$
DECLARE
  r record;
  expected text;
...
BEGIN
  FOR r IN
    ...
    FROM public.ukpubs_profiles p
  LOOP
    expected := public.ukpubs_slugify_username(r.display_name);
    -- Skip if already correct (exact or numbered suffix of the expected base)
    IF r.username = expected OR r.username ~ ('^' || expected || '[0-9]+$') THEN
      CONTINUE;
    END IF;
    -- Skip intentional machine fallbacks
    IF r.username LIKE 'user_%' AND char_length(r.username) >= 8 THEN
      CONTINUE;
    END IF;
    ...
    FOR i IN 0..24 LOOP
      IF i = 0 THEN
        candidate := expected;
      ELSE
        candidate := left(expected, 20) || (i + 1)::text;
      END IF;
